...
const SingleAccountPage = () => {
  ...
  useEffect(() => {
    const singleAccount = data.filter(
    ...
    <Row className="pt-4">
      <Col sm={12} md={9} className="pt-3">
        ...
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>{accountData.account.name}</Breadcrumb.Item>
        </Breadcrumb>
        ...
        <div className="d-flex align-items-center">
          ...
          <AccountLabel role={accountData.account.role} className={"ms-4"} />
        </div>
              ...
              {accountData.account.properties.length === 0 ? (
                ...
                accountData.account.properties.map(({ id, property }) => (
                  <Col key={id} sm={12} md={4}>
                    <Property property={property} user={accountData.account} id={id}  />
                  </Col>
                ))
              )}
            </Row>
